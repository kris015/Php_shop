    ...
    <div class="container">
        ...
        <div class="row">
                    ...
                    if($db->error()==""){
                        if($db->num_rows($rez)>0){
                            ...
                            while($red=$db->fetch_assoc($rez)){
                                ...
                                if(login())echo "<button class='btn btn-success mt-2' onclick='dodajProizvod({$red['id']})'>Dodaj u korpu</button>";
                                echo "</div><hr>";
            ...
            <div class="row">
                <div class="col-12">
                    <h3>Komentari</h3>
                    <input type="checkbox" id="cbKomentar" onclick="prikaziFormu();"> Da li želite da ostavite komentar?
                    <form action="proizvod.php?id=<?= $id?>" method="post" id="frmKomentari"  style="display:none">
            ...
            $rez=$db->query($upit);
            while($red=$db->fetch_object($rez)){
                echo "slike.push('slikeProizvoda/{$red->imeSlike}');";
            }
            echo "</script>";
            ?>
            <div id="galerija"></div>
